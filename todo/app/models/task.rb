...
class Task < ApplicationRecord
  ...
  scope :search_with_user_id, ->(user_id) { where(user_id: user_id) }
  ...
  def self.search(params, sort_column, user_id:)
    return Task.all.order(sort_column => :desc) unless params
    Task.title_name_partial_match(params[:title]).status_equal(params[:status]).search_with_user_id(user_id).order(sort_column => :desc)
  end
end
