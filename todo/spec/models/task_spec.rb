...
RSpec.describe Task, type: :model do
  ...
  let(:user) { build(:user) }
  subject { build(:task, title: title, description: description, status: status, due_date: due_date, user: user) }
  ...
  describe 'when search' do
    let!(:task1) { create(:task, title: 'タスク１', description: 'タスク１詳細', status: Task.statuses[:todo], due_date: Time.zone.local(2020, 1, 1, 0, 0), user: user) }
    let!(:task2) { create(:task, title: 'タスク２', description: 'タスク２詳細', status: Task.statuses[:doing], due_date: Time.zone.local(2021, 1, 1, 0, 0), user: user) }
