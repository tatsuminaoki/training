class Pagination {
  ...
  constructor() {
    this.elPagination = document.getElementById('pagination');
  }
  ...
  init(params, reset = false) {
    ...
    this._disabledPaginationBySuffix(this.currentPage);
  }
  ...
  _createPageElement(idSuffix, text) {
    ...
    elLink.onclick = () => {
      ...
      .then(json => {
        this._enablePaginationBySuffix(this.currentPage);
        this.currentPage = page;
        this._disabledPaginationBySuffix(page);
    ...
    this.elPagination.appendChild(elLi);
  }
  ...
  _disabledPaginationBySuffix(idSuffix) {
    ...
    if(idSuffix == 1) {
      this._disabledPaginationByHash(this._getPrevElements());
    } else if (idSuffix == this.maxPage) {
      this._disabledPaginationByHash(this._getNextElements());
    }
  }

  _enablePaginationBySuffix(idSuffix) {
    const elLi   = document.getElementById('page-' + idSuffix);
    const elLink = document.getElementById('page-link-' + idSuffix);
  ...
  _disabledPaginationByHash(hash) {
