...
module UsersHelper
  def sort_users_link(column, order)
    ...
    link_to_if(!match_sort_condition?(column, order), order_display, order_by: order_by, order: order)
  end
  ...
  def sort_user_tasks_link(column, order)
    ...
    order_display = 'ASC'.casecmp?(order) ? I18n.t('links.ASC') : I18n.t('links.DESC')
    link_to_if(!match_sort_condition?(column, order), order_display, admin_user_path(name: params[:name], status: params[:status], order_by: order_by, order: order))
  end
