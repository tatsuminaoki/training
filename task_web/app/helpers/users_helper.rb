...
module UsersHelper
  def sort_users_link(column, order)
    ...
    link_to_if(!match_sort?(column, order), order_display(order), order_by: order_by, order: order)
  end
  ...
  def sort_user_tasks_link(column, order)
    ...
    link_to_if(!match_sort?(column, order), order_display(order),
               admin_user_path(name: params[:name], status: params[:status], order_by: order_by, order: order, label_ids: params[:label_ids]))
  end
