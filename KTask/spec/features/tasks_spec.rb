...
RSpec.feature 'Tasks', type: :feature do
  ...
  scenario 'タスクの作成日順並べ替えテスト' do
    create(:task, title: 'task1', created_at: '2018-9-29', user_id: pseudo_login_user.id)
    create(:task, title: 'task2', created_at: '2018-9-30', user_id: pseudo_login_user.id)
    visit root_path
  ...
  scenario '一覧画面で終了期限で整列されていること' do
    create(:task, title: 'task1', end_time: '2018-9-29 12:10:10', user_id: pseudo_login_user.id)
    create(:task, title: 'task2', end_time: '2018-9-30 12:10:10', user_id: pseudo_login_user.id)
    visit root_path
  ...
  scenario '一覧画面でタイトル名と状態で検索' do
    create(:task, title: 'task1', status: 'do', user_id: pseudo_login_user.id)
    create(:task, title: 'task2', status: 'done', user_id: pseudo_login_user.id)
    visit root_path
  ...
  scenario 'タスク五つのページネーション確認' do
    create_list(:task, 6 , user_id: pseudo_login_user.id)
    visit root_path
    titles = page.all('td.title')
